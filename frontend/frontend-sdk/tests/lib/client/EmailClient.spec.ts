...
describe("EmailClient.create()", () => {
  it("should create a email address", async () => {
    ...
    await expect(createResponse).resolves.toBe(undefined);
...
describe("EmailClient.update()", () => {
  it("should list email addresses", async () => {
    const response = new Response(new XMLHttpRequest());
    response.ok = true;

    jest.spyOn(emailClient.client, "patch").mockResolvedValue(response);
    const update = await emailClient.update(emailID, true);
    expect(emailClient.client.patch).toHaveBeenCalledWith(
      `/emails/${emailID}`,
      { is_primary: true }
    );
  ...
  `(
    ...
    async ({ status, error }) => {
      ...
      jest.spyOn(emailClient.client, "patch").mockResolvedValueOnce(response);

      const email = emailClient.update(emailID, true);
      await expect(email).rejects.toThrow(error);
    }
  );

  it("should throw error on API communication failure", async () => {
    emailClient.client.patch = jest
      .fn()
      .mockRejectedValue(new Error("Test error"));

    const user = emailClient.update(emailID, true);
    await expect(user).rejects.toThrowError("Test error");
  });
});
