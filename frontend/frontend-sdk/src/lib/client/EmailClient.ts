...
class EmailClient extends Client {
  ...
  /**
   * Updates the specified email address.
   *
   * @param {string} emailID - The ID of the email address to be updated
   * @param {string} isPrimary - Indicates the specified email address should be marked as primary.
   * @return {Promise<void>}
   * @throws {UnauthorizedError}
   * @throws {RequestTimeoutError}
   * @throws {TechnicalError}
   * @see https://docs.hanko.io/api/public#tag/Email-Management/operation/createEmail
   */
  async update(emailID: string, isPrimary: boolean): Promise<void> {
